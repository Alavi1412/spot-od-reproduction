...
- Runtime import support for in-place script execution: `src/`, `scripts/_bootstrap.py`, `scripts/__init__.py`, `pyproject.toml`, `requirements.txt`, and the repository `README.md`.
...
The package verifier extracts the ZIP into a temporary directory and runs `python scripts/run_trajectory_candidate_graph_selector_poc.py --help` from that extracted root. This fast smoke checks import-time package wiring without starting model training.
