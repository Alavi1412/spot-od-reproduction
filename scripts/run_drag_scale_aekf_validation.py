#!/usr/bin/env python
"""DSA-EKF validation tuning over the predeclared grid (Loop 45).

Runs the Drag-Scale Adaptive EKF on a validation seed disjoint from the
held-out test seed and from every training/model-selection split used in the
manuscript. Evaluates the predeclared small grid of hyperparameters and writes
a selection artifact recording the lowest-mean-observed-step-RMSE grid point.

The selection artifact is later loaded by the held-out test harness
``scripts/run_drag_scale_aekf_force_mismatch.py`` so the test population
never enters the hyperparameter selection.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--trajectories", type=int, default=None,
                   help="Overrides predeclared validation_n_trajectories.")
    p.add_argument("--epoch-unix", type=float, default=1_736_640_000.0)
    p.add_argument("--diurnal-alpha", type=float, default=0.30)
    return p
...
def main() -> int:
    ...
    for gp in grid:
        ...
        print(f"[validation] {gp['label']}: mean={mean_rmse:.4f} m  n_finite={int(finite.size)}", flush=True)
    t_grid = time.perf_counter() - t_grid

    chosen = min(results, key=lambda r: r["validation_mean_observed_step_rmse_m"])

    payload: dict[str, Any] = {
        "scenario": "drag_scale_aekf_validation",
        "schema_version": "drag_scale_aekf_validation_v1",
