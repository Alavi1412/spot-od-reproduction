#!/usr/bin/env python
"""Run a dropout-severity sensitivity sweep with retraining and reevaluation."""
...
def format_level_tag(value: float) -> str:
    token = f"{value:.6g}"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--dropout-levels", type=str, default="0.0,0.02,0.05")
    p.add_argument("--epochs", type=int, default=60)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--vary-seed-by-level", action="store_true")
    p.add_argument("--output-dir", type=str, default="results/dropout_sweep")
...
def build_variant_config(
    ...
    cfg["seed"] = int(seed)
    cfg["simulation"]["measurement_noise"]["random_dropout_prob"] = float(dropout_prob)
...
def extract_summary_row(dropout_prob: float, metrics_path: Path) -> dict[str, float]:
...
def main() -> None:
    ...
    levels = parse_float_list(args.dropout_levels)
    ...
    for idx, level in enumerate(levels):
        tag = format_level_tag(level)
        run_dir = root_out / f"drop_{tag}"
        ...
        rows.append(extract_summary_row(dropout_prob=level, metrics_path=metrics_path))

    df = pd.DataFrame(rows).sort_values("dropout_prob")
    summary_csv = root_out / "dropout_summary.csv"
    df.to_csv(summary_csv, index=False)
    dump_json({"levels": rows}, root_out / "dropout_summary.json")
