"""Loop-160 additional internal prospective independently seeded observed-step replication.

This script manages the local freeze record and supporting utilities for a new
additional independently seeded internal prospective replication of the primary
observed-step endpoint. The actual scenario shards are run with the existing
``build_observed_step_prospective_replication.py`` builder and merged with the
loop160-specific ``merge-shards`` subcommand of this script (NOT the generic
...
freeze
    ...
    ``results/observed_step_internal_prospective_replication_loop160/preregistration.json``
...
print-shard-commands
    ...
    the canonical loop-160 base seed, rule path, and output paths.  Does not
    run the experiment.  Also prints the loop160-specific merge-shards command.
...
merge-shards
    Validate and merge the three loop-160 scenario shard JSON files into the
...
OUT_DIR = ROOT / "results" / "observed_step_internal_prospective_replication_loop160"
PREREGISTRATION_PATH = OUT_DIR / "preregistration.json"
ARTIFACT_PATH = OUT_DIR / "observed_step_internal_prospective_replication_loop160.json"
...
# Canonical loop-160 parameters.
BASE_SEED = 1160000
NUM_REALIZATIONS = 32
TRAJECTORIES = 24
BOOTSTRAP_SAMPLES = 5000
SCHEMA_VERSION = "observed_step_internal_prospective_replication_loop160_rule_v1"
    ...
    record: dict[str, Any] = {
        ...
        "endpoint_hierarchy": {
            ...
            "endpoint_selection_note": (
                ...
                "support record and K=32 central replication preceded this "
                "additional independently seeded internal replication"
            ),
        ...
        "interpretation_boundary": (
            "This record defines an additional internal prospective "
            "independently seeded replication under the established "
            "observed-step hierarchy and the same frozen decision predicate "
            "used by the K=32 central replication. The rule is fixed locally "
            "before any loop-160 realization is generated or evaluated. This "
            "record is not an external preregistration, does not constitute "
            "public-reference validation or operational validation, and "
            "permits no model selection, tuning, or retraining after the "
            "record is written."
        ),
        "prior_replications": {
            "k32_central": {
...
def print_shard_commands(args_device: str = "auto") -> None:
    ...
    print(
        "# Loop-160 additional internal prospective replication shard commands.\n"
        "# Run freeze first, then execute these commands sequentially or in parallel,\n"
        "# then run the loop160-specific merge-shards subcommand (see below).\n"
        "#\n"
        "# Each shard scores one scenario with 32 independent realizations and\n"
        "# 24 trajectories per realization (base seed 1160000).\n"
    )
    ...
    print(
        "# Merge command (after all shards complete) -- use the loop160-specific\n"
...
# ---------------------------------------------------------------------------
# Loop-160-specific shard merge
# ---------------------------------------------------------------------------


class MergeValidationError(ValueError):
    """Raised when loop-160 shards cannot safely be merged."""


def _validate_loop160_shard(
    path: Path,
    payload: dict[str, Any],
) -> dict[str, Any]:
    """Validate a single loop-160 shard payload and return the scenario row.
    ...
    if not isinstance(scenarios_list, list) or len(scenarios_list) != 1:
        n = len(scenarios_list) if isinstance(scenarios_list, list) else type(scenarios_list)
        raise MergeValidationError(
...
def _build_merge_summary(scenarios: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    verdict = (
        "larger independent observed-step replication under the frozen rule: "
...
def merge_shards_loop160(
    ...
    ----------
    shard_paths:
        Explicit shard file paths.  Defaults to the three canonical loop-160
    ...
    for path in shard_paths:
        ...
        scenario = _validate_loop160_shard(path, payload)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    merge_p = subparsers.add_parser(
        "merge-shards",
        help=(
            "Validate and merge the loop-160 scenario shards into the canonical "
    ...
    merge_p.add_argument(
        ...
        help=(
            ...
            "Defaults to the three canonical loop-160 shard files."
        ),
    )
    merge_p.add_argument(
        "--out-path",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.subcommand == "print-shard-commands":
        print_shard_commands(args_device=args.device)
        return 0
