...
\addlinespace
Exploratory audited-family learned negative on the observed-step endpoint & Table~\ref{tab:main_k32_replication}, S-Table~\ref{tab:observed_step_powered_stress_replication}, S-Table~\ref{tab:observed_step_internal_prospective_replication_k96_allscenario} & No positives for the originally audited learned family under the per-scenario best-tuned-classical rule at $K=32$; no positives for that same family in the stress-only $K=96$ check and all-scenario $K=96$ seed-disjoint replication & Supported as a secondary internal frozen-rule negative for the originally audited family under the selected observed-step endpoint & Not external preregistration, not a claim about all learned OD systems, not a statement about the post-existing-manuscript GraphAnchorPairGate PoC, and not operational POD validation \\
\addlinespace
